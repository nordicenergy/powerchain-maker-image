...
docker run -ti --rm -v ${HOME}:/root -v $(pwd):/git nordicenergy/git clone https://github.com/nordicenergy/powerchain-maker.git
docker run -ti --rm -v ${HOME}:/root -v $(pwd):/git -w /git/powerchain alpine/git checkout $powerchain_version
docker run -it --rm -v $(pwd)/vamise:/powerchain -w /powerchain golang:1.10.2 make all
